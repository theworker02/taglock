...
Compile-time confidence for Go's runtime metadata, and compatibility intelligence for the contracts that metadata creates. TagLock statically analyzes Go struct tags as one serialized contract. It finds today's collisions, unsafe exposure, type errors, and namespace drift, then can snapshot those external representations and detect changes
